...
class SupabaseClient(context: Context) {
    ...
    private fun authenticate(path: String, email: String, password: String) {
        val body = JSONObject().put("email", email).put("password", password)
        val result = request("POST", "/auth/v1/$path", body.toString(), false)
        val json = JSONObject(result)
        val token = json.optString("access_token")
        if (token.isEmpty()) throw IOException(json.optString("msg", "Authentication failed"))
    ...
    private fun request(method: String, path: String, content: String?, auth: Boolean,
        ...
        http.newCall(builder.build()).execute().use { response ->
            ...
            if (!response.isSuccessful) {
                val message = try { JSONObject(text).optString("message", text) }
